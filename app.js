...
function registrarOcorrencia() {
    ...
    if (!tipo || !motivo) {
        mockAction("Por favor, preencha o tipo e o motivo da ocorrência.");
        return;
    }

    // Salvar no localStorage para mostrar na tela de detalhes depois
    localStorage.setItem('j3g_last_occurrence', JSON.stringify({
        tipo: tipo,
        motivo: motivo,
        obs: document.getElementById('obsOcorrencia').value,
        data: new Date().toLocaleString()
    }));

    mockAction("Ocorrência registrada com sucesso!");
    
    // Redirecionar de volta após um pequeno delay
    setTimeout(() => {
        window.location.href = 'detalhes.html';
    }, 1500);
}

// Chamar esta função na tela de detalhes para mostrar ocorrências se existirem
function checkOccurrences() {
    const section = document.getElementById('ocorrenciaRegistradaSection');
    const data = localStorage.getItem('j3g_last_occurrence');
    
    if (section && data) {
        const occ = JSON.parse(data);
        document.getElementById('ocTipo').innerText = occ.tipo;
        document.getElementById('ocMotivo').innerText = occ.motivo;
        document.getElementById('ocObs').innerText = occ.obs || 'Sem observações adicionais.';
        section.style.display = 'block';
    }
}

// Chamar checkOccurrences se estiver na página de detalhes
